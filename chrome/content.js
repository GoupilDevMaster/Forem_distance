...
async function logError(ctx, err, extra) {
  ...
  try {
    const s = await chrome.storage.local.get("errorLog");
  ...
  chrome.runtime.sendMessage({ type: "errorLogged" }).catch(() => {});
}
  ...
  while (queue.length > 0) {
    ...
    try {
      ...
      if (!isCached && routingService.id !== "osrm") {
        chrome.runtime.sendMessage({ type: "trackRequest", serviceId: routingService.id })
          .catch(() => {});
      }
...
function resetAllBadges() {
  ...
  chrome.runtime.sendMessage({ type: "progressUpdate", completed: 0, total: 0 }).catch(() => {});
...
async function init() {
  ...
  const result = await chrome.storage.local.get(["departure", "routingService"]);
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  if (message.type === "getProgress") {
    sendResponse({ completed: completedJobs, total: totalJobs });
    return true;
  }
  if (message.type === "refresh") {
    chrome.storage.local.get(["departure", "routingService"]).then((result) => {
      departure = result.departure || null;
      routingService = result.routingService || { id: "osrm", apiKey: "" };
      if (!departure) {
        showNoBanner();
