...
function onServiceChange(counts) {
  ...
  if (service !== "osrm") {
    ...
    if (counts !== undefined) {
      ...
      chrome.storage.local.get("requestCounts").then((s) =>
...
async function saveAdvanced() {
  ...
  await chrome.storage.local.set({ routingService: { id, apiKey }, warnThreshold });
...
async function loadSavedCity() {
  const result = await chrome.storage.local.get(["departure", "routingService", "requestCounts", "warnThreshold"]);
...
async function buildReportMarkdown() {
  const s = await chrome.storage.local.get(["departure", "routingService", "errorLog"]);
...
chrome.tabs.query({ active: true, currentWindow: true }).then((tabs) => {
  if (tabs.length > 0 && tabs[0].url && /leforem\.be|ictjob\.be|indeed\.com/.test(tabs[0].url)) {
    chrome.tabs.sendMessage(tabs[0].id, { type: "getProgress" })
